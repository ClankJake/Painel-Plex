...
// --- Card de Resumo ---
function createSummaryCard(id, icon, label, value, colorClass) {
    // **CORREÇÃO**: Revertido para um estilo mais simples com cores sólidas e text-white.
    // Removidas as classes bg-opacity, backdrop-blur, etc.
    return `
...
export function renderSummaryCards(summary) {
    ...
    // **CORREÇÃO**: Alterado de classes de gradiente para cores sólidas (ex: bg-blue-500).
    // Estas classes são padrão e têm maior probabilidade de funcionar.
    dom.summaryCardsContainer.innerHTML = `
        ${createSummaryCard('active-streams-card', '<svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>', i18n.activeStreams, summary.active_streams, 'bg-blue-500')}
        ...
        ${createSummaryCard('monthly-revenue-card', '<svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01" /></svg>', i18n.monthlyRevenue, formatCurrency(summary.monthly_revenue), 'bg-green-500')}
        ${createSummaryCard('upcoming-renewals-card', '<svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>', i18n.upcomingRenewals, summary.upcoming_renewals, 'bg-yellow-500')}
    `;
}
...
export function renderCharts(summary) {
    ...
    if (dom.revenueCanvas) {
        ...
        if (state.monthlyRevenueChart) {
            ...
            state.monthlyRevenueChart.update('none'); // Update sem animação
        } else {
    ...
    if (dom.userStatusCanvas) {
        ...
        if (state.userStatusChart) {
            ...
            state.userStatusChart.update('none'); // Update sem animação
        } else {
    ...
    if (sessions && sessions.length > 0) {
        ...
        sessions.forEach(s => {
            ...
            if (!card) {
                ...
                // Corresponde ao estilo do seu HTML (sem borda, bg-white, etc.)
...
export function renderTerminationLogs(logs) {
    ...
    dom.auditLogContainer.innerHTML = logs.map(log => createTerminationLogRow(log)).join('');
...
export function prependTerminationLog(log) {
    ...
    if (placeholder) { // Remove o placeholder se existir
        placeholder.remove();
    }
    const newRowHtml = createTerminationLogRow(log);
    dom.auditLogContainer.insertAdjacentHTML('afterbegin', newRowHtml);
...
// --- Modal de Seleção de Usuário (LÓGICA ATUALIZADA) ---
export function updateSpecificUserLabel(count) {
    const { i18n } = state;
    const countSpan = document.getElementById('target-specific-count');
    if (!countSpan) return;
    countSpan.textContent = (count > 0) ? ` (${count} ${i18n.selected})` : '';
}

/**
 * Abre e preenche o modal de seleção de usuário que já existe no HTML.
 */
export function openUserSelectionModal() {
    const { i18n } = state;
    const modalElement = dom.userSelectionModal; // Obtém o modal do config
    if (!modalElement) {
        console.error("Elemento do modal #userSelectionModal não encontrado.");
        return;
    }

    // Encontra as partes internas do modal
    const modalBody = modalElement.querySelector('.modal-body');
    const modalFooter = modalElement.querySelector('.modal-footer');
    const closeBtn = modalElement.querySelector('.modal-close');
    const modalTitle = modalElement.querySelector('.modal-title');

    if (!modalBody || !modalFooter || !closeBtn || !modalTitle) {
        console.error("Estrutura interna do modal está incompleta.");
        return;
    }

    // Define IDs únicos para os elementos internos
    ...
    // Define o HTML para o corpo e rodapé
    const modalBodyHtml = `
    ...
    // Preenche o modal
    modalTitle.textContent = i18n.selectUsers;
    modalBody.innerHTML = modalBodyHtml;
    modalFooter.innerHTML = modalFooterHtml;

    // Obtém os elementos *depois* de injetá-los
    const searchInput = modalBody.querySelector(`#${searchInputId}`);
    const userList = modalBody.querySelector(`#${listId}`);
    const selectAll = modalBody.querySelector(`#${selectAllId}`);
    const deselectAll = modalBody.querySelector(`#${deselectAllId}`);
    const countSpan = modalBody.querySelector(`#${countId}`);
    const confirmBtn = modalFooter.querySelector(`#${confirmBtnId}`);
    const cancelBtn = modalFooter.querySelector(`#${cancelBtnId}`);

    const tempSelectedIds = new Set(state.selectedUserIds);

    const updateModalCount = () => {
        if (!countSpan) return;
        countSpan.textContent = `${tempSelectedIds.size} ${i18n.selected}`;
    };
    ...
    const filterModalUserList = (allUsers) => {
    ...
    // Função para fechar o modal (agora apenas o esconde)
    const closeModal = (forceResetRadio = false) => {
        modalElement.classList.add('hidden');
        if (forceResetRadio && state.selectedUserIds.size === 0) {
    ...
    // Anexa listeners aos botões do modal
    if (confirmBtn) {
        confirmBtn.onclick = () => {
    ...
    getUsersForSelection()
        .then(allUsers => {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', () => filterModalUserList(allUsers));
            }
            if (selectAll) {
        ...
        .catch(error => {
            if (userList) userList.innerHTML = `<p class="text-red-400 text-sm p-2">${error.message}</p>`;
        });
