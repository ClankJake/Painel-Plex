/**
 * Módulo de Configuração
 * Extrai e exporta URLs e textos de internacionalização (i18n)
 * do script tag no HTML para serem usados em toda a aplicação da página de configurações.
 */

import { lerConfiguracaoDoScript } from '../utils.js';

export const { urls, i18n } = lerConfiguracaoDoScript('settings-script');
export const fieldMap = {
    'APP_TITLE': { type: 'text', default: 'Painel Plex' },
    'APP_BASE_URL': { type: 'text', default: 'http://127.0.0.1:5000' },
    'ENABLE_LINK_SHORTENER': { type: 'checkbox', default: true },
    'DAYS_TO_REMOVE_BLOCKED_USER': { type: 'number', default: 0 },
    'EXPIRATION_NOTIFICATION_TIME': { type: 'text', default: '09:00' },
    'BLOCK_REMOVAL_TIME': { type: 'text', default: '02:00' },
    'UNIVERSAL_EXPIRATION_ENABLED': { type: 'checkbox', default: false },
    'UNIVERSAL_EXPIRATION_TIME': { type: 'text', default: '23:59' },
    'CLEANUP_PENDING_PAYMENTS_ENABLED': { type: 'checkbox', default: true },
    'CLEANUP_PENDING_PAYMENTS_DAYS': { type: 'number', default: 3 },
    'CLEANUP_TIME': { type: 'text', default: '03:00' },
    'IMAGE_CACHE_CLEANUP_ENABLED': { type: 'checkbox', default: true },
    'IMAGE_CACHE_MAX_AGE_DAYS': { type: 'number', default: 30 },
    'IMAGE_CACHE_CLEANUP_TIME': { type: 'text', default: '04:00' },
    'SHORT_LINK_CLEANUP_ENABLED': { type: 'checkbox', default: true },
    'SHORT_LINK_MAX_AGE_DAYS': { type: 'number', default: 30 },
    'TELEGRAM_ENABLED': { type: 'checkbox', default: false },
    'DISCORD_ENABLED': { type: 'checkbox', default: false },
    'WEBHOOK_ENABLED': { type: 'checkbox', default: false },
    'WEBHOOK_URL': { type: 'text', default: '' },
    'WEBHOOK_AUTHORIZATION_HEADER': { type: 'text', default: '' },
    'WEBHOOK_EXPIRATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_RENEWAL_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_REACTIVATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_TRIAL_END_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_TRIAL_EXTENDED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_CREDENTIALS_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_PASSWORD_RESET_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_BOT_TOKEN': { type: 'password', default: '' },
    'TELEGRAM_CHAT_ID': { type: 'text', default: '' },
    'DAYS_TO_NOTIFY_EXPIRATION': { type: 'number', default: 2 },
    'TELEGRAM_EXPIRATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_RENEWAL_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_REACTIVATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_TRIAL_END_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_TRIAL_EXTENDED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_CREDENTIALS_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_PASSWORD_RESET_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_WEBHOOK_URL': { type: 'text', default: '' },
    'DISCORD_EXPIRATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_RENEWAL_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_REACTIVATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_TRIAL_END_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_TRIAL_EXTENDED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_CREDENTIALS_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_PASSWORD_RESET_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'plex_url_display': { type: 'text', readonly: true, key: 'PLEX_URL' },
    'TAUTULLI_URL': { type: 'text', default: '' },
    'TAUTULLI_API_KEY': { type: 'password', default: '' },
    'JELLYFIN_URL': { type: 'text', default: '' },
    'JELLYFIN_API_KEY': { type: 'password', default: '' },
    'EFI_ENABLED': { type: 'checkbox', default: false },
    'MERCADOPAGO_ENABLED': { type: 'checkbox', default: false },
    'GATES2B_ENABLED': { type: 'checkbox', default: false },
    'EFI_CLIENT_ID': { type: 'text', default: '' },
    'EFI_CLIENT_SECRET': { type: 'password', default: '' },
    'EFI_CERTIFICATE': { type: 'text', default: '' },
    'EFI_SANDBOX': { type: 'checkbox', default: false },
    'EFI_PIX_KEY': { type: 'text', default: '' },
    'EFI_USE_MTLS': { type: 'checkbox', default: true },
    'EFI_WEBHOOK_HMAC_SECRET': { type: 'text', default: '' },
    'MERCADOPAGO_ACCESS_TOKEN': { type: 'password', default: '' },
    'MERCADOPAGO_WEBHOOK_SECRET': { type: 'password', default: '' },
    'MERCADOPAGO_MIN_AMOUNT': { type: 'decimal', default: 1 },
    'GATES2B_AUTH_TOKEN': { type: 'password', default: '' },
    'GATES2B_MIN_AMOUNT': { type: 'decimal', default: 3 },
    'RENEWAL_PRICE': { type: 'text', default: '10.00' },
    'PAYMENT_LINK_GRACE_PERIOD_DAYS': { type: 'number', default: 7 },
    'PRICE_SCREEN_1': { type: 'price', key: '1' },
    'PRICE_SCREEN_2': { type: 'price', key: '2' },
    'PRICE_SCREEN_3': { type: 'price', key: '3' },
    'PRICE_SCREEN_4': { type: 'price', key: '4' },
    'PRICE_SCREEN_5': { type: 'price', key: '5' },
    'PRICE_SCREEN_6': { type: 'price', key: '6' },
    'OVERSEERR_ENABLED': { type: 'checkbox', default: false },
    'DISABLE_ONLINE_MEDIA_SOURCES_ON_CLAIM': { type: 'checkbox', default: false },
    'OVERSEERR_URL': { type: 'text', default: '' },
    'OVERSEERR_API_KEY': { type: 'password', default: '' },
    'ACHIEVEMENT_MOVIE_MARATHON_BRONZE': { type: 'number', default: 5 },
    'ACHIEVEMENT_MOVIE_MARATHON_SILVER': { type: 'number', default: 10 },
    'ACHIEVEMENT_MOVIE_MARATHON_GOLD': { type: 'number', default: 20 },
    'ACHIEVEMENT_SERIES_BINGER_BRONZE': { type: 'number', default: 20 },
    'ACHIEVEMENT_SERIES_BINGER_SILVER': { type: 'number', default: 50 },
    'ACHIEVEMENT_SERIES_BINGER_GOLD': { type: 'number', default: 100 },
    'ACHIEVEMENT_TIME_TRAVELER_BRONZE': { type: 'number', default: 3 },
    'ACHIEVEMENT_TIME_TRAVELER_SILVER': { type: 'number', default: 5 },
    'ACHIEVEMENT_TIME_TRAVELER_GOLD': { type: 'number', default: 7 },
    'ACHIEVEMENT_DIRECTOR_FAN_BRONZE': { type: 'number', default: 3 },
    'ACHIEVEMENT_DIRECTOR_FAN_SILVER': { type: 'number', default: 5 },
    'ACHIEVEMENT_DIRECTOR_FAN_GOLD': { type: 'number', default: 7 },
    'ACHIEVEMENT_NIGHT_OWL_BRONZE': { type: 'number', default: 3 },
    'ACHIEVEMENT_NIGHT_OWL_SILVER': { type: 'number', default: 10 },
    'ACHIEVEMENT_NIGHT_OWL_GOLD': { type: 'number', default: 25 },
    'ACHIEVEMENT_PIONEER_BRONZE': { type: 'number', default: 1 },
    'ACHIEVEMENT_PIONEER_SILVER': { type: 'number', default: 5 },
    'ACHIEVEMENT_PIONEER_GOLD': { type: 'number', default: 15 },
    'ACHIEVEMENT_PIONEER_WINDOW_HOURS': { type: 'number', default: 48 },
    'XP_PER_MINUTE_WATCHED': { type: 'decimal', default: 1 },
    'BULK_SEND_INTERVAL_SECONDS': { type: 'decimal', default: 0.2 },
    'WHATSAPP_ENABLED': { type: 'checkbox', default: false },
    'WHATSAPP_PROVIDER': { type: 'text', default: 'evolution' },
    'WHATSAPP_API_URL': { type: 'text', default: '' },
    'WHATSAPP_API_KEY': { type: 'password', default: '' },
    'WHATSAPP_INSTANCE': { type: 'text', default: '' },
    'WHATSAPP_DEFAULT_COUNTRY_CODE': { type: 'text', default: '55' },
    'WHATSAPP_CUSTOM_PAYLOAD_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_EXPIRATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_RENEWAL_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_REACTIVATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_TRIAL_END_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_TRIAL_EXTENDED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_CREDENTIALS_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_PASSWORD_RESET_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_MEDIA_PENDING_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_MEDIA_PENDING_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_MEDIA_PENDING_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_MEDIA_APPROVED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_MEDIA_APPROVED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_MEDIA_APPROVED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_MEDIA_AVAILABLE_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_MEDIA_AVAILABLE_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_MEDIA_AVAILABLE_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_MEDIA_DECLINED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_MEDIA_DECLINED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_MEDIA_DECLINED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'TELEGRAM_MEDIA_FAILED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_MEDIA_FAILED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_MEDIA_FAILED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WHATSAPP_BULK_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    // Notificações push (navegador e celular). As chaves VAPID não estão
    // aqui de propósito: são geradas pelo painel, não editadas na página.
    'PUSH_ENABLED': { type: 'checkbox', default: false },
    'PUSH_VAPID_SUBJECT': { type: 'text', default: '' },
    'PUSH_ADMIN_PAYMENTS': { type: 'checkbox', default: true },
    'PUSH_ADMIN_INVITES': { type: 'checkbox', default: true },
    'PUSH_ADMIN_MEDIA_REQUESTS': { type: 'checkbox', default: true },
    'PUSH_EXPIRATION_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_EXPIRATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_RENEWAL_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_RENEWAL_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_REACTIVATION_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_REACTIVATION_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_TRIAL_END_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_TRIAL_END_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_TRIAL_EXTENDED_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_TRIAL_EXTENDED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_BULK_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_BULK_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_REQUEST_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_REQUEST_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_PENDING_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_PENDING_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_APPROVED_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_APPROVED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_AVAILABLE_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_AVAILABLE_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_DECLINED_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_DECLINED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PUSH_MEDIA_FAILED_TITLE_TEMPLATE': { type: 'text', default: '' },
    'PUSH_MEDIA_FAILED_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'PRORATION_ENABLED': { type: 'checkbox', default: false },
    'PRORATION_MIN_CHARGE': { type: 'decimal', default: 2 },
    'PRORATION_MIN_DAYS': { type: 'number', default: 3 },
    'PRORATION_FREE_BELOW_MINIMUM': { type: 'checkbox', default: true },
    'REFERRAL_ENABLED': { type: 'checkbox', default: false },
    'REFERRAL_REWARD_TYPE': { type: 'text', default: 'days' },
    'REFERRAL_REWARD_DAYS': { type: 'number', default: 7 },
    'REFERRAL_REWARD_CREDIT': { type: 'decimal', default: 5 },
    'REFERRAL_DEFAULT_INVITE_CODE': { type: 'text', default: '' },
    'REFERRAL_MAX_REWARDS_PER_USER': { type: 'number', default: 0 },
    'XP_BONUS_PER_COMPLETED_ITEM': { type: 'number', default: 20 },
    'XP_COMPLETION_THRESHOLD_PERCENT': { type: 'number', default: 90 },
    'XP_RESET_ENABLED': { type: 'checkbox', default: false },
    'RECOMMENDATIONS_ENABLED': { type: 'checkbox', default: true },
    'RECOMMENDATIONS_HISTORY_DAYS': { type: 'number', default: 180 },
    'RECOMMENDATIONS_MIN_PERCENT_WATCHED': { type: 'number', default: 25 },
    'RECOMMENDATIONS_MIN_CO_OCCURRENCE': { type: 'number', default: 2 },
    'RECOMMENDATIONS_MAX_SECTIONS': { type: 'number', default: 4 },
    'RECOMMENDATIONS_ITEMS_PER_SECTION': { type: 'number', default: 8 },
    'RECOMMENDATIONS_RESPECT_PRIVACY': { type: 'checkbox', default: true },
    'RECOMMENDATIONS_GENRE_LOOKUP_LIMIT': { type: 'number', default: 40 },
    'TELEGRAM_BULK_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'DISCORD_BULK_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'WEBHOOK_BULK_MESSAGE_TEMPLATE': { type: 'textarea', default: '' },
    'STREAM_CHECK_INTERVAL_SECONDS': { type: 'number', default: 15 },
    'SCREEN_LIMIT_TERMINATION_STRATEGY': { type: 'text', default: 'oldest' },
    'FORCE_STREAM_TERMINATION': { type: 'checkbox', default: false },
    'BACKUP_ENABLED': { type: 'checkbox', default: false },
    'BACKUP_TIME': { type: 'text', default: '05:00' },
    'BACKUP_MAX_COUNT': { type: 'number', default: 7 },
    'TERMINATION_MSG_BLOCKED_MANUAL': { type: 'textarea', default: '' },
    'TERMINATION_MSG_BLOCKED_EXPIRED': { type: 'textarea', default: '' },
    'TERMINATION_MSG_BLOCKED_TRIAL_EXPIRED': { type: 'textarea', default: '' },
    'TERMINATION_MSG_SCREEN_LIMIT': { type: 'textarea', default: '' },
};

