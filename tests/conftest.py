...
class FakeDataManager:
    ...
    def get_user_profile(self, media_user_id):
        return self.profiles.get(int(media_user_id))

    def set_user_profile(self, media_user_id, profile_data):
        profile = self.profiles.setdefault(int(media_user_id), {"media_user_id": int(media_user_id)})
    ...
    def get_users_referred_by(self, media_user_id):
        return [p for p in self.profiles.values() if p.get("referred_by") == int(media_user_id)]
    ...
    def consume_referral_credit(self, media_user_id, amount):
        profile = self.profiles.get(int(media_user_id))
        if not profile:
            return 0.0
    ...
    def get_reserved_referral_credit(self, media_user_id, exclude_txid=None):
        return float(self.reserved_credit.get(int(media_user_id), 0.0))
    ...
    def count_rewarded_referrals(self, media_user_id):
        return len([
            ...
            if p.get("referred_by") == int(media_user_id) and p.get("referral_rewarded")
        ])

    def user_has_completed_payment(self, media_user_id):
        return int(media_user_id) in self.paid_users
    ...
    def get_blocked_user(self, media_user_id):
        return self.blocked.get(int(media_user_id))

    # --- Conquistas ---
    def get_unlocked_achievements(self, media_user_id):
        return set(self.achievements.get(int(media_user_id), set()))

    def add_unlocked_achievements(self, media_user_id, username, achievements_to_add):
        atuais = self.achievements.setdefault(int(media_user_id), set())
    ...
    def has_user_used_coupon(self, media_user_id, code):
        return (int(media_user_id), code) in self.used_coupons
    ...
    def has_user_pending_coupon_charge(self, media_user_id, code):
        return (int(media_user_id), code) in self.pending_coupon_charges
